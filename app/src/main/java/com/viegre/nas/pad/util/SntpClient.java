...
/**
 * Created by レインマン on 2021/03/08 16:09 with Android Studio.
 */
public class SntpClient {
	...
	public boolean requestTime(String host, int timeout) {
		...
		try {
			...
			//             = (2 * skew)/2 = skew
			LogUtils.dTag(TAG, "Request time form ntp server success, " + address.toString() + " ,roundTripTime: " + roundTripTime);
			if (BuildConfig.DEBUG) {
				LogUtils.dTag(TAG, "round trip: " + roundTripTime + "ms, " + "clock offset: " + clockOffset + "ms");
			}
...
//            }
			LogUtils.eTag(TAG, "Request time from ntp server failed ,msg: " + e.getMessage());
			return false;
		} finally {
			if (socket != null) {
				socket.close();
