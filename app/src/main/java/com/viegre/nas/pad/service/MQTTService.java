...
//		initFileWatcher();
	...
	private void initAndServer() {
		mServer = AndServer.webServer(this).port(8080).timeout(15, TimeUnit.SECONDS).listener(new Server.ServerListener() {
			@Override
								...
								ftpFavoritesListMap.put("favoritesPathList",
								                        LitePal.where("state = ?", FtpFileEntity.State.NORMAL)
								                               ...
								                               .filter(ftpFileEntity -> ftpFileEntity.getPickSet()
								                                                                     .contains(mqttMsgEntity.getFromId()) && FileUtils
										                               .isFileExists(ftpFileEntity.getPath()))
								                               .collect(Collectors.toList()));
							...
							public Void doInBackground() {
								...
								ftpGetBanListMap.put("banList",
								                     LitePal.where("state = ?", FtpFileEntity.State.NORMAL)
								                            ...
								                            .find(FtpFileEntity.class)
								                            .stream()
								                            .filter(ftpFileEntity -> ftpFileEntity.getBanSet()
								                                                                  .contains(getBanPhoneNum) && FileUtils.isFileExists(
										                            ftpFileEntity.getPath()))
								                            .collect(Collectors.toList()));
