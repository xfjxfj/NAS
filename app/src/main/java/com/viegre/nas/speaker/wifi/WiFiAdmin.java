package com.viegre.nas.speaker.wifi;
...
/**
 * Created by Djangoogle on 2020/11/30 16:30 with Android Studio.
 */
...
//        }
	...
	public int getCipherType(String ssid) {
		...
		for (ScanResult scResult : list) {
			...
			if (!TextUtils.isEmpty(scResult.SSID) && (scResult.SSID.trim().equalsIgnoreCase("\"" + ssid + "\"") || scResult.SSID.trim().equalsIgnoreCase(ssid))) {
	...
	public boolean ifWifiExsits(String ssid) {
		...
		for (int i = 0; i < mWifiList.size(); i++) {
			if (mWifiList.get(i).SSID.equalsIgnoreCase("\"" + ssid + "\"") || mWifiList.get(i).SSID.equalsIgnoreCase(ssid)) {
				return true;
			}
		}
		return false;
	...
	public boolean isConnect(String ssid) {
		...
		if (wifiInfo != null) {
			return wifiInfo.getSSID().equalsIgnoreCase("\"" + ssid + "\"") || wifiInfo.getSSID().equalsIgnoreCase(ssid);
		}
		return false;
	}
}
