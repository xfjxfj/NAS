...
public class ClassifyView extends FrameLayout {
	...
	private final int[] mMainLocation = new int[2];
	private final int[] mSubLocation = new int[2];
	 ...
	 */
	private final Queue<Integer> mInMergeQueue = new LinkedList<>();
	/**
	 * 触发滑动距离
	 */
...
//                        inMainRegion = true;
						...
						pressedView.startDrag(ClipData.newPlainText(DESCRIPTION, MAIN), new ClassifyDragShadowBuilder(pressedView), mSelected, 0);
					}
				}
			}
		});
						...
						if ((mRegion & IN_MAIN_REGION) != 0) {
							if (inMergeState) {
								...
								mDragView.animate()
								         ...
								         .scaleX(scaleX)
								         .scaleY(scaleY)
								         .setListener(mMergeAnimListener)
...
//            L.d("Main onDrag X:%1$s,Y:%2$s", x, y);
	...
	private final AnimatorListenerAdapter mMergeAnimListener = new AnimatorListenerAdapter() {
		@Override
	...
	private void doStartDragWithAnimation(final RecyclerView recyclerView, final View selected, final int selectedPosition, final float targetX, final float targetY,
	                                      @NonNull final int[] fixWindowLocation, final BaseCallBack callBack) {
	...
	private void doRecoverAnimation() {
		...
		if ((mRegion & IN_SUB_REGION) != 0) {
			...
			if (holder == null) {
				...
				PropertyValuesHolder xOffset = PropertyValuesHolder.ofFloat("x",
				                                                            mSubLocation[0] + mSubContainer.getLeft() + holder.itemView.getLeft());
	...
	private final AnimatorListenerAdapter mRecoverAnimatorListener = new AnimatorListenerAdapter() {
		@Override
		public void onAnimationEnd(Animator animation) {
			for (DragListener listener : mDragListeners) {
	...
	private boolean scrollIfNecessary() {
		...
		if (lm.canScrollHorizontally()) {
			...
			if (mDx < 0 && leftDiff < 0) {
				...
				final int rightDiff = curX + mSelected.getWidth() + mEdgeWidth - (recyclerView.getWidth() - recyclerView.getPaddingRight());
				if (rightDiff > 0) {
					scrollX = rightDiff;
				}
			}
		...
		if (lm.canScrollVertically()) {
			...
			if (mDy < 0 && topDiff < 0) {
				...
				final int bottomDiff = curY + mSelected.getHeight() + mEdgeWidth - (recyclerView.getHeight() - recyclerView.getPaddingBottom());
				if (bottomDiff > 0) {
					scrollY = bottomDiff;
				}
			}
		...
		if (scrollX != 0) {
			scrollX = interpolateOutOfBoundsScroll(recyclerView, mSelected.getWidth(), scrollX, recyclerView.getWidth(), scrollDuration);
		}
		if (scrollY != 0) {
			scrollY = interpolateOutOfBoundsScroll(recyclerView, mSelected.getHeight(), scrollY, recyclerView.getHeight(), scrollDuration);
		}
	...
	private void moveIfNecessary(View view) {
		...
		//如果移动范围在自身范围内
		if (Math.abs(y - view.getTop()) < view.getHeight() * 0.5f && Math.abs(x - view.getLeft()) < view.getWidth() * 0.5f) {
			return;
		}
	...
	protected View chooseTarget(View selected, List<View> swapTargets, int curX, int curY) {
		...
		for (int i = 0; i < targetsSize; i++) {
			...
			final int score = Math.abs(target.getLeft() - curX) + Math.abs(target.getTop() - curY) + Math.abs(target.getBottom() - bottom) + Math.abs(
...
//            }
	...
	private final ElevationHelper mElevationHelper = new ElevationHelper();
