...
public class InsertAbleGridView extends ViewGroup implements CanMergeView {
	...
	private void init(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mBagDrawable.setOutlineStyle(a.getColor(R.styleable.InsertAbleGridView_OutlineColor, 0),
		...
		if (childCount > 0) {
			if (childCount == 1) {
				...
				if (michaelViewHolder == null) {
					...
					getChildAt(0).layout(getPaddingLeft() + mOutLinePadding,
					                     getPaddingTop() + mOutLinePadding,
					                     getPaddingLeft() + mOutLinePadding + width,
					                     getPaddingTop() + mOutLinePadding + height);
				} else {
	...
	private ValueAnimator createConvertAnimator(final View view) {
		...
		PropertyValuesHolder right = PropertyValuesHolder.ofInt("right",
		                                                        view.getRight(),
		                                                        getPaddingLeft() + mInnerPadding + mOutLinePadding + getItemWidth(width));
		PropertyValuesHolder top = PropertyValuesHolder.ofInt("top", view.getTop(), getPaddingTop() + mInnerPadding + mOutLinePadding);
		PropertyValuesHolder bottom = PropertyValuesHolder.ofInt("bottom",
		                                                         view.getBottom(),
		                                                         getPaddingTop() + mInnerPadding + mOutLinePadding + getItemHeight(height));
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (widthMode != MeasureSpec.EXACTLY) { widthMeasureSpec = MeasureSpec.makeMeasureSpec(widthSize, MeasureSpec.EXACTLY); }
		if (heightMode != MeasureSpec.EXACTLY) { heightMeasureSpec = MeasureSpec.makeMeasureSpec(heightSize, MeasureSpec.EXACTLY); }
		int childCount = getChildCount();
