...
public abstract class RoSafFile<T> extends AbstractFile {
	...
	static final String[] SAF_QUERY_COLUMNS = {DocumentsContract.Document.COLUMN_DOCUMENT_ID, DocumentsContract.Document.COLUMN_DISPLAY_NAME, DocumentsContract.Document.COLUMN_LAST_MODIFIED, DocumentsContract.Document.COLUMN_SIZE, DocumentsContract.Document.COLUMN_MIME_TYPE, DocumentsContract.Document.COLUMN_FLAGS,};

	public RoSafFile(ContentResolver contentResolver, Uri startUrl, String absPath, PftpdService pftpdService) {
		// this c-tor is to be used for start directory
		super(absPath, null, 0, 0, false, false, false, pftpdService);
		this.contentResolver = contentResolver;
		this.startUrl = startUrl;

		try {
			Cursor cursor = contentResolver.query(startUrl, SAF_QUERY_COLUMNS, null, null, null);
			try {
				initByCursor(cursor);
			} finally {
				closeQuietly(cursor);
	...
	public RoSafFile(ContentResolver contentResolver, Uri startUrl, String docId, String absPath, boolean exists, PftpdService pftpdService) {
		// this c-tor is to be used for FileSystemView.getFile()
		super(absPath, null, 0, 0, false, exists, false, pftpdService);
		this.contentResolver = contentResolver;
		this.startUrl = startUrl;

		if (exists) {
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
				Uri uri = DocumentsContract.buildDocumentUriUsingTree(startUrl, docId);

				Cursor cursor = contentResolver.query(uri, SAF_QUERY_COLUMNS, null, null, null);
				cursor.moveToNext();
				try {
					initByCursor(cursor);
				} finally {
	...
	public RoSafFile(ContentResolver contentResolver, Uri startUrl, Cursor cursor, String absPath, PftpdService pftpdService) {
	...
	protected abstract T createFile(ContentResolver contentResolver, Uri startUrl, Cursor cursor, String absPath, PftpdService pftpdService);
...
//        }
	...
	public List<T> listFiles() {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			...
			Uri childrenUri = DocumentsContract.buildChildDocumentsUriUsingTree(startUrl, documentId);
			Cursor childCursor = contentResolver.query(childrenUri, SAF_QUERY_COLUMNS, null, null, null);
			try {
				while (childCursor.moveToNext()) {
					String absPath = this.absPath.endsWith("/") ? this.absPath + childCursor.getString(CURSOR_INDEX_NAME) : this.absPath + "/" + childCursor
							.getString(CURSOR_INDEX_NAME);
	...
	public OutputStream createOutputStream(long offset) throws IOException {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			Uri uri = DocumentsContract.buildDocumentUriUsingTree(startUrl, documentId);
