...
public class RootSshFile extends RootFile<SshFile> implements SshFile {
	...
	public RootSshFile(Shell.Interactive shell, LsOutputBean bean, String absPath, PftpdService pftpdService, Session session, RootSshFileSystemView fileSystemView) {
	...
	public Object getAttribute(Attribute attribute, boolean followLinks) throws IOException {
		...
		switch (attribute) {
			...
			case Permissions:
				...
				if (bean.isUserWritable()) { tmp.add(Permission.UserWrite); }
				if (bean.isUserExecutable()) { tmp.add(Permission.UserExecute); }
				if (bean.isGroupReadable()) { tmp.add(Permission.GroupRead); }
				if (bean.isGroupWritable()) { tmp.add(Permission.GroupWrite); }
				if (bean.isGroupExecutable()) { tmp.add(Permission.GroupExecute); }
				if (bean.isOtherReadable()) { tmp.add(Permission.OthersRead); }
				if (bean.isOtherWritable()) { tmp.add(Permission.OthersWrite); }
				if (bean.isOtherExecutable()) { tmp.add(Permission.OthersExecute); }
