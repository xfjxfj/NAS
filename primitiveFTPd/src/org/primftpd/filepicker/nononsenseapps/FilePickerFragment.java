...
public class FilePickerFragment extends AbstractFilePickerFragment<File> {
	...
	protected boolean hasPermission(@NonNull File path) {
		return PackageManager.PERMISSION_GRANTED == ContextCompat.checkSelfPermission(getContext(), Manifest.permission.WRITE_EXTERNAL_STORAGE);
	}
...
//        }
	...
	public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
		...
		if (permissions.length == 0) {
			...
			if (PackageManager.PERMISSION_GRANTED == grantResults[0]) {
				...
				Toast.makeText(getContext(), R.string.permissionRequired, Toast.LENGTH_SHORT).show();
				// Treat this as a cancel press
				if (mListener != null) {
					mListener.onCancelled();
				}
	...
	public void onNewFolder(@NonNull final String name) {
		...
		if (folder.mkdir()) {
			...
			Toast.makeText(getActivity(), R.string.filepickerCreateFolderError, Toast.LENGTH_SHORT).show();
		}
	}
