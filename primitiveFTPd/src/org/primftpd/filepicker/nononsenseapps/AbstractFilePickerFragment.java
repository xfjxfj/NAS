...
public abstract class AbstractFilePickerFragment<T> extends Fragment implements LoaderManager.LoaderCallbacks<SortedList<T>>, NewItemFragment.OnNewFolderListener, LogicHandler<T> {
	 ...
	 */
	public void setArgs(@Nullable
	                    final String startPath, final int mode, final boolean allowMultiple, final boolean allowDirCreate, final boolean allowExistingFile, final boolean singleClick) {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		view.findViewById(R.id.filepicker_button_cancel).setOnClickListener(new View.OnClickListener() {
			@Override
		...
		view.findViewById(R.id.filepicker_button_ok).setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(final View v) {
				onClickOk(v);
			}
		});
		view.findViewById(R.id.filepicker_button_ok_newfile).setOnClickListener(new View.OnClickListener() {
			@Override
	...
	protected void configureItemDecoration(@NonNull LayoutInflater inflater, @NonNull RecyclerView recyclerView) {
		final TypedArray attributes = getActivity().obtainStyledAttributes(new int[]{R.attr.filepickerListItemDivider});
	...
	public void onClickOk(@NonNull View view) {
		...
		if ((allowMultiple || mode == MODE_FILE) && (mCheckedItems.isEmpty() || getFirstCheckedItem() == null)) {
			if (mToast == null) {
				mToast = Toast.makeText(getActivity(), R.string.filepickerSelectSomethingFirst, Toast.LENGTH_SHORT);
			}
			mToast.show();
			return;
		}
		...
		if (mode == MODE_NEW_FILE) {
			...
			if (filename.startsWith("/")) {
				...
				result = toUri(getPath(Utils.appendPath(getFullPath(mCurrentPath), filename)));
			}
	...
	public boolean isCheckable(@NonNull final T data) {
		...
		if (isDir(data)) {
			checkable = ((mode == MODE_DIR && allowMultiple) || (mode == MODE_FILE_AND_DIR && allowMultiple));
		} else {
			// File
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		if (mCurrentPath == null) {
			if (savedInstanceState != null) {
				...
				allowExistingFile = savedInstanceState.getBoolean(KEY_ALLOW_EXISTING_FILE, allowExistingFile);
	...
	protected void setModeView() {
		...
		if (!nf && singleClick) {
			getActivity().findViewById(R.id.filepicker_button_ok).setVisibility(View.GONE);
		}
	}

	@Override
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		inflater.inflate(R.menu.filepicker_actions, menu);

		MenuItem item = menu.findItem(R.id.filepicker_action_createdir);
		item.setVisible(allowCreateDir);
	}

	@Override
	public boolean onOptionsItemSelected(MenuItem menuItem) {
		if (R.id.filepicker_action_createdir == menuItem.getItemId()) {
			Activity activity = getActivity();
			if (activity instanceof AppCompatActivity) {
				NewFolderFragment.showDialog(((AppCompatActivity) activity).getSupportFragmentManager(), AbstractFilePickerFragment.this);
			}
			return true;
		} else {
			return false;
	...
	public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
		...
		switch (viewType) {
			case LogicHandler.VIEWTYPE_HEADER:
				v = LayoutInflater.from(getActivity()).inflate(R.layout.filepicker_listitem_dir, parent, false);
				return new HeaderViewHolder(v);
			case LogicHandler.VIEWTYPE_CHECKABLE:
				v = LayoutInflater.from(getActivity()).inflate(R.layout.filepicker_listitem_checkable, parent, false);
	...
	protected boolean isItemVisible(final T file) {
		return (isDir(file) || (mode == MODE_FILE || mode == MODE_FILE_AND_DIR) || (mode == MODE_NEW_FILE && allowExistingFile));
	}
