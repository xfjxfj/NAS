...
public class ServerServiceHandler extends Handler {
	...
	protected ServerServiceHandler(Looper looper, AbstractServerService service, String logName) {
		super(looper);
	...
	protected void handleStart(AbstractServerService service) {
		if (service.getServer() == null) {
			...
			if (started && service.getServer() != null) {
				PowerManager powerMgr = (PowerManager) service.getSystemService(AbstractServerService.POWER_SERVICE);
				...
				Notification notification = ServicesStartStopUtil.updateNonActivityUI(service,
				                                                                      true,
				                                                                      service.prefsBean,
	...
	protected void handleStop(AbstractServerService service) {
		...
		service.stopSelf();
		ServicesStartStopUtil.updateNonActivityUI(service, false, service.prefsBean, null, null);
	}

	private synchronized void obtainWakeLock(PowerManager powerMgr, boolean takeWakeLock) {
		...
		if (wakeLock == null && takeWakeLock) {
			...
			wakeLock = powerMgr.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK, APP_NAME + ":wakelock");
			wakeLock.acquire();
		} else {
			if (takeWakeLock) {
