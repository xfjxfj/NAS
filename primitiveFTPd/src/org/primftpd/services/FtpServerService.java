		...
		serverFactory.setFileSystem(new FileSystemFactory() {
			...
			public FileSystemView createFileSystemView(User user) {
				if (quickShareBean != null) {
					...
					return new QuickShareFtpFileSystemView(new File(quickShareBean.getPathToFile()), user, FtpServerService.this);
				} else {
					switch (prefsBean.getStorageType()) {
						...
						case SAF:
							return new SafFtpFileSystemView(getApplicationContext(),
							                                Uri.parse(prefsBean.getSafUrl()),
							                                getContentResolver(),
							                                FtpServerService.this,
							                                user);
						case RO_SAF:
							return new RoSafFtpFileSystemView(Uri.parse(prefsBean.getSafUrl()), getContentResolver(), FtpServerService.this, user);
					}
				}
				return null;
			}
