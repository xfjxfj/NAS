...
public class SshServerService extends AbstractServerService {
	...
	protected boolean launchServer(final Shell.Interactive shell) {
		...
		if (StringUtils.isNotEmpty(prefsBean.getPassword()) || prefsBean.isAnonymousLogin()) {
			...
			sshServer.setPasswordAuthenticator(new PasswordAuthenticator() {
				...
				public boolean authenticate(String username, String password, ServerSession session) {
					Authentication authentication = prefsBean.isAnonymousLogin() ? new AnonymousAuthentication() : new UsernamePasswordAuthentication(
							username,
							password);
		...
		if (prefsBean.isPubKeyAuth()) {
			String[] keyPaths = new String[]{Defaults.pubKeyAuthKeyPath(getApplicationContext()), Defaults.PUB_KEY_AUTH_KEY_PATH_OLD, Defaults.PUB_KEY_AUTH_KEY_PATH_OLDER,};
			...
			if (!pubKeys.isEmpty()) {
				...
				Toast.makeText(getApplicationContext(), getText(R.string.couldNotReadKeyAuthKey), Toast.LENGTH_SHORT).show();
			}
		}
		...
		sshServer.setFileSystemFactory(new FileSystemFactory() {
			...
			public FileSystemView createFileSystemView(Session session) {
				if (quickShareBean != null) {
					...
					return new QuickShareSshFileSystemView(new File(quickShareBean.getPathToFile()), SshServerService.this, session);
				} else {
					switch (prefsBean.getStorageType()) {
						...
						case SAF:
							return new SafSshFileSystemView(getApplicationContext(),
							                                Uri.parse(prefsBean.getSafUrl()),
							                                getContentResolver(),
							                                SshServerService.this,
							                                session);
						case RO_SAF:
							return new RoSafSshFileSystemView(Uri.parse(prefsBean.getSafUrl()), getContentResolver(), SshServerService.this, session);
					}
				}
				return null;
			}
