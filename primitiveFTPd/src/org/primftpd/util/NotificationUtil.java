...
public class NotificationUtil {
	...
	public static void createStatusbarNotification(Context ctxt, Notification notification, int id) {
		NotificationManager notiMgr = (NotificationManager) ctxt.getSystemService(Context.NOTIFICATION_SERVICE);
	...
	private static Notification.Builder createStubNotification(Context ctxt, int text, int actionText, String channelId, QuickShareBean quickShareBean) {
		...
		CharSequence contentTitle = quickShareBean != null ? String.format(ctxt.getResources().getString(R.string.quickShareInfoNotification),
		                                                                   quickShareBean.filename()) : ctxt.getText(R.string.notificationTitle);
		...
		Notification.Builder builder = new Notification.Builder(ctxt).setTicker(tickerText)
	...
	private static void addAction(Context ctxt, Notification.Builder builder, PendingIntent pendingIntent, int text, int iconId) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
			// TODO check icon for android 7
			Icon icon = Icon.createWithResource(ctxt, iconId);
			Notification.Action stopAction = new Notification.Action.Builder(icon, ctxt.getString(text), pendingIntent).build();
	...
	public static void createStartStopNotification(Context ctxt) {
		...
		Notification.Builder builder = createStubNotification(ctxt, R.string.toggleService, R.string.toggleService, START_STOP_CHANNEL_ID, null);
	...
	public static Notification createStatusbarNotification(Context ctxt, PrefsBean prefsBean, KeyFingerprintProvider keyFingerprintProvider, QuickShareBean quickShareBean) {
		LOGGER.debug("createStatusbarNotification()");

		Notification.Builder builder = createStubNotification(ctxt,
	...
	private static String buildLongText(Context ctxt, PrefsBean prefsBean, KeyFingerprintProvider keyFingerprintProvider) {
	...
	public static Notification createDownloadNotification(Context ctxt, String filename, String path, boolean canceled, boolean finished, long currentBytes, long size) {
		...
		// use main icon as large one
		Bitmap largeIcon = BitmapFactory.decodeResource(ctxt.getResources(), R.drawable.outline_cloud_download_black_48);
