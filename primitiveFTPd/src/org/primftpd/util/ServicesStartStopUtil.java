...
public class ServicesStartStopUtil {
	...
	public static void startServers(Context context, PrefsBean prefsBean, KeyFingerprintProvider keyFingerprintProvider, PrimitiveFtpdActivity activity) {
		startServers(context, prefsBean, keyFingerprintProvider, activity, null);
	}

	public static void startServers(Context context, PrefsBean prefsBean, KeyFingerprintProvider keyFingerprintProvider, PrimitiveFtpdActivity activity, QuickShareBean quickShareBean) {
		LOGGER.trace("startServers()");

		if (!isPasswordOk(prefsBean)) {
			Toast.makeText(context, R.string.haveToSetAuthMechanism, Toast.LENGTH_LONG).show();
...
//			}
			...
			if (prefsBean.getServerToStart().startSftp()) {
				...
				if (keyPresent) {
					...
					try {
						...
						Toast.makeText(context, "could not start sftp server, " + e.getMessage(), Toast.LENGTH_SHORT);
					}
				}
			}
			if (continueServerStart) {
				if (prefsBean.getServerToStart().startFtp()) {
					...
					try {
						...
						Toast.makeText(context, "could not start ftp server, " + e.getMessage(), Toast.LENGTH_SHORT);
					}
				}
			}
		}
	...
	protected static Intent createFtpServiceIntent(Context context, PrefsBean prefsBean, KeyFingerprintProvider keyFingerprintProvider, QuickShareBean quickShareBean) {
	...
	protected static Intent createSshServiceIntent(Context context, PrefsBean prefsBean, KeyFingerprintProvider keyFingerprintProvider, QuickShareBean quickShareBean) {
	...
	public static Notification updateNonActivityUI(Context ctxt, boolean serverRunning, PrefsBean prefsBean, KeyFingerprintProvider keyFingerprintProvider, QuickShareBean quickShareBean) {
		LOGGER.trace("updateNonActivityUI()");
		Notification notification = null;
		updateWidget(ctxt, serverRunning);
		if (serverRunning) {
			notification = NotificationUtil.createStatusbarNotification(ctxt, prefsBean, keyFingerprintProvider, quickShareBean);
		} else {
