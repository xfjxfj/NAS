...
class SoundPoolPlayback(private val context: Context?) {
	...
	private val maxVolume = am?.getStreamMaxVolume(AudioManager.STREAM_MUSIC)
			?.toFloat() ?: 0F
	private val currentVolume = am?.getStreamVolume(AudioManager.STREAM_MUSIC)
			?.toFloat() ?: 0F
	...
	private fun loadForFile(list: MutableList<File>) {
		val temp = mutableListOf<Int>()
		list.filter { it.exists() && it.isFile }
				.forEach { file ->
	...
	fun playSound(index: Int,
		...
		return songIdList.getOrNull(index)
				?.let {
					volumeMap[it] = SoundPoolVolume(left, right)
					soundPool?.play(it, left, right, priority, loop, rate)
							.orDef()
				}
				.orDef()
	}
