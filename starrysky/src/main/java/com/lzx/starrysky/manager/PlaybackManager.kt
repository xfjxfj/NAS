...
class PlaybackManager(private val provider: MediaSourceProvider,
	...
	fun attachPlayerCallback(serviceCallback: PlaybackServiceCallback) {
		...
		StarrySky.getBinder()
				?.setSessionToken(sessionManager.getMediaSession())
	...
	fun onPrepareById(songId: String) {
		...
		val list = mediaQueue.getCurrSongList()
	...
	fun onStopByTimedOff(time: Long, isPause: Boolean, finishCurrSong: Boolean) {
		StarrySky.getBinder()
				?.onStopByTimedOff(time, isPause, finishCurrSong)
	}
	...
	private fun updatePlaybackState(currPlayInfo: SongInfo?, errorMsg: String?, state: Int) {
		...
		StarrySky.getBinder()
				?.onChangedNotificationState(currPlayInfo, newState,
