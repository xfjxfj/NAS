...
class PlayerControl(
	...
	fun addInterceptor(interceptor: ISyInterceptor): PlayerControl {
		val noSame = interceptors.filter { it.getTag() == interceptor.getTag() }
				.isNullOrEmpty()
	 ...
	 * 获取当前播放的歌曲信息
	 */
	fun getNowPlayingSongInfo(): SongInfo? = playbackManager.player()
			?.getCurrPlayInfo()
	 ...
	 * 以ms为单位获取当前缓冲的位置。
	 */
	fun getBufferedPosition(): Long = playbackManager.player()
			?.bufferedPosition()
			.orDef()
	 ...
	 * 获取播放位置 毫秒为单位。
	 */
	fun getPlayingPosition(): Long = playbackManager.player()
	 ...
	 */
	fun getPlaybackSpeed(): Float = playbackManager.player()
			?.getPlaybackSpeed()
			.orDef()
	 ...
	 * 获取音量
	 */
	fun getVolume(): Float = playbackManager.player()
			?.getVolume()
			.orDef()
	 ...
	 * 获取媒体时长，单位毫秒
	 */
	fun getDuration(): Long = playbackManager.player()
			?.duration()
			.orDef()
	 ...
	 * 获取 AudioSessionId
	 */
	fun getAudioSessionId(): Int = playbackManager.player()
			?.getAudioSessionId()
			.orDef()
	...
	fun querySongInfoInLocal(context: Context): List<SongInfo> {
		...
		while (cursor.moveToNext()) {
			...
			val songId = if (song.songUrl.isNotEmpty()) song.songUrl.md5() else System.currentTimeMillis()
					.toString()
					.md5()
			song.songId = songId
			songInfos.add(song)
	...
	fun setOnPlayProgressListener(listener: OnPlayProgressListener) {
		val pkgActivityName = StarrySky.getStackTopActivity()
				?.toString()
		pkgActivityName?.let {
