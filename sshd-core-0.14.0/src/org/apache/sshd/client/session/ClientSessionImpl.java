...
public class ClientSessionImpl extends AbstractSession implements ClientSession {
	...
	public SshFuture switchToNoneCipher() throws IOException {
		if (!(currentService instanceof AbstractConnectionService) || !((AbstractConnectionService) currentService).getChannels().isEmpty()) {
		...
		if (kexState.compareAndSet(KEX_STATE_DONE, KEX_STATE_INIT)) {
			...
			if (!serverProposal[SshConstants.PROPOSAL_ENC_ALGS_CTOS].matches("(^|.*,)none($|,.*)") || !serverProposal[SshConstants.PROPOSAL_ENC_ALGS_STOC]
					.matches("(^|.*,)none($|,.*)")) {
				reexchangeFuture.setValue(new SshException("Server does not support none cipher"));
			} else if (!clientProposal[SshConstants.PROPOSAL_ENC_ALGS_CTOS].matches("(^|.*,)none($|,.*)") || !clientProposal[SshConstants.PROPOSAL_ENC_ALGS_STOC]
					.matches("(^|.*,)none($|,.*)")) {
