...
public abstract class AbstractSession extends CloseableUtils.AbstractInnerCloseable implements Session {
	...
	protected Closeable getInnerCloseable() {
		return builder().parallel(getServices()).close(ioSession).build();
	}
			...
			if (decoderState == 0) {
				...
				if (decoderBuffer.available() > inCipherSize) {
					...
					if (decoderLength < 5 || decoderLength > (256 * 1024)) {
						...
						throw new SshException(SshConstants.SSH2_DISCONNECT_PROTOCOL_ERROR, "Invalid packet length: " + decoderLength);
					}
	...
	protected String[] createProposal(String hostKeyTypes) {
		return new String[]{NamedFactory.Utils.getNames(factoryManager.getKeyExchangeFactories()), hostKeyTypes, NamedFactory.Utils.getNames(
				factoryManager.getCipherFactories()), NamedFactory.Utils.getNames(factoryManager.getCipherFactories()), NamedFactory.Utils.getNames(
				factoryManager.getMacFactories()), NamedFactory.Utils.getNames(factoryManager.getMacFactories()), NamedFactory.Utils.getNames(
				factoryManager.getCompressionFactories()), NamedFactory.Utils.getNames(factoryManager.getCompressionFactories()), "", ""};
	}
	...
	protected void negotiate() {
		...
		for (int i = 0; i < SshConstants.PROPOSAL_MAX; i++) {
			...
			if (guess[i] == null) {
				String message = "Unable to negotiate key exchange for " + paramName + " (client: " + clientParamValue + " / server: " + serverParamValue + ")";
				...
				if (log.isTraceEnabled()) {
					log.trace("Kex: negotiate(" + paramName + ") guess=" + guess[i] + " (client: " + clientParamValue + " / server: " + serverParamValue);
				}
			}
		}
		negotiated = guess;
		...
		log.debug("Kex: server->client {} {} {}",
		          new Object[]{negotiated[SshConstants.PROPOSAL_ENC_ALGS_STOC], negotiated[SshConstants.PROPOSAL_MAC_ALGS_STOC], negotiated[SshConstants.PROPOSAL_COMP_ALGS_STOC]});
		log.debug("Kex: client->server {} {} {}",
		          new Object[]{negotiated[SshConstants.PROPOSAL_ENC_ALGS_CTOS], negotiated[SshConstants.PROPOSAL_MAC_ALGS_CTOS], negotiated[SshConstants.PROPOSAL_COMP_ALGS_CTOS]});
	}
	 ...
	 */
	protected static class PendingWriteFuture extends DefaultSshFuture<IoWriteFuture> implements IoWriteFuture, SshFutureListener<IoWriteFuture> {
