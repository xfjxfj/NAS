	...
	public class Socks4 extends Proxy {
		...
		protected void onMessage(Buffer buffer) throws IOException {
			if (channel == null) {
				...
				String host = getUByte(buffer) + "." + getUByte(buffer) + "." + getUByte(buffer) + "." + getUByte(
						buffer);
	...
	public class Socks5 extends Proxy {
		...
		protected void onMessage(Buffer buffer) throws IOException {
			if (authMethods == null) {
				...
				String host;
				if (type == 0x01) {
					host = getUByte(buffer) + "." + getUByte(buffer) + "." + getUByte(buffer) + "." + getUByte(
							buffer);
				} else if (type == 0x03) {
					host = getBLString(buffer);
				} else if (type == 0x04) {
					host = Integer.toHexString(getUShort(buffer)) + ":" + Integer.toHexString(getUShort(buffer)) + ":" + Integer.toHexString(getUShort(
							buffer)) + ":" + Integer.toHexString(getUShort(buffer)) + ":" + Integer.toHexString(getUShort(buffer)) + ":" + Integer.toHexString(
							getUShort(buffer)) + ":" + Integer.toHexString(getUShort(buffer)) + ":" + Integer.toHexString(getUShort(buffer));
				} else {
