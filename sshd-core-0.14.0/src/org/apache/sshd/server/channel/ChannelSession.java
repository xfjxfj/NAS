...
public class ChannelSession extends AbstractServerChannel {
	...
	protected Closeable getInnerCloseable() {
		return builder().sequential(new CommandCloseable(), new GracefulChannelCloseable()).parallel(asyncOut, asyncErr).build();
	}
	...
	protected boolean handlePtyReq(Buffer buffer) throws IOException {
		...
		if (log.isDebugEnabled()) {
			log.debug("pty for channel {}: term={}, size=({} - {}), pixels=({}, {}), modes=[{}]",
			          new Object[]{id, term, tColumns, tRows, tWidth, tHeight, ptyModes});
		}
	...
	protected boolean handleX11Forwarding(Buffer buffer) throws IOException {
		...
		String display = service.createX11Display(buffer.getBoolean(), buffer.getString(), buffer.getString(), buffer.getInt());
		if (display == null) {
			return false;
		}
