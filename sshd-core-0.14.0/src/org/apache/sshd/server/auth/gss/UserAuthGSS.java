...
public class UserAuthGSS extends AbstractUserAuth {
	...
	protected Boolean doAuth(Buffer buffer, boolean initial) throws Exception {
		...
		if (initial) {
			...
			if (!(msg == SshConstants.SSH_MSG_USERAUTH_INFO_RESPONSE || msg == SshConstants.SSH_MSG_USERAUTH_GSSAPI_MIC && context.isEstablished())) {
			...
			if (context.isEstablished()) {
				...
				msgbuf.putString(username.getBytes(StandardCharsets.UTF_8));
