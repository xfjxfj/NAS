...
public class LunarCalendar {
	...
	public static final int[] lunarToSolar(int year, int month, int monthDay, boolean isLeapMonth) {
		...
		for (i = 1; i < month; i++) {
			if ((LUNAR_INFO[year - MIN_YEAR] & (0x80000 >> (i - 1))) == 0) { dayOffset += 29; } else { dayOffset += 30; }
		}
		...
		if (leapMonth != 0) {
			if (month > leapMonth || (month == leapMonth && isLeapMonth)) {
				if ((LUNAR_INFO[year - MIN_YEAR] & (0x80000 >> (month - 1))) == 0) { dayOffset += 29; } else { dayOffset += 30; }
			}
		}

		if (dayOffset > 366 || (year % 4 != 0 && dayOffset > 365)) {
			year += 1;
			if (year % 4 == 1) { dayOffset -= 366; } else { dayOffset -= 365; }
		}
		...
		for (i = 1; i < 13; i++) {
			...
			if (iPos >= dayOffset) {
				...
				if (dayOffset > iPos) { solarInfo[2] = dayOffset - iPos; } else if (dayOffset == iPos) {
					if (year % 4 == 0 && i == 2) { solarInfo[2] = DAYS_BEFORE_MONTH[i] - DAYS_BEFORE_MONTH[i - 1] + 1; } else {
	...
	private static int daysInLunarMonth(int year, int month) {
		if ((LUNAR_INFO[year - MIN_YEAR] & (0x100000 >> month)) == 0) { return 29; } else { return 30; }
	}
